...
//		taskNoEva.getWaitingComment(myApplication.getUserId(), true);
		...
		client.get(url, new AsyncHttpResponse(){
			...
			public void onSuccess(int statusCode, String content) {
				...
				if(HttpStatus.SC_OK == statusCode){
					...
					ArrayList<Cart> noEvaCarts = parseEvaJson.getWaitCommGoods();
					if(isSuccess && null != noEvaCarts){
						showView(noEvaCarts);
					} else {
						Toast.makeText(EvaluationActivity.this, "暂无数据", Toast.LENGTH_SHORT).show();
					}
				}
			}
	...
	private void showView(ArrayList<Cart> waitCommGoods){
		...
		for (int i = 0; i < length; i++) {
			...
			View view = LayoutInflater.from(this).inflate(R.layout.evaluation_item, null);
			...
			evaluation_item_sum.setText(getResources().getString(R.string.unit) + mCart.getPrice());
			...
			evaluation_item_evaluation.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					// 评价
					intent.setClass(EvaluationActivity.this, PersonEvaluationActivity.class);
