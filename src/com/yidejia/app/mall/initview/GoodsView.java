...
public class GoodsView {
	...
	public void initGoodsView(ProductBaseInfo info){
		...
		//销售额
		TextView selled_num_text = (TextView) view.findViewById(R.id.selled_num_text);
		selled_num_text.setText(info.getSalledAmmount());
		//评论数
		TextView emulate_num_text = (TextView) view.findViewById(R.id.emulate_num_text);
		emulate_num_text.setText(info.getCommentAmount());
		//晒单数
		TextView show_num_text = (TextView) view.findViewById(R.id.show_num_text);
		show_num_text.setText(info.getShowListAmount());
		//品牌
		TextView brand_name_text = (TextView) view.findViewById(R.id.brand_name_text);
		brand_name_text.setText(info.getBrands());
		//商品编号
		TextView product_id_num_text = (TextView) view.findViewById(R.id.product_id_num_text);
		product_id_num_text.setText(info.getProductNumber());
		//规格
		TextView standard_content_text = (TextView) view.findViewById(R.id.standard_content_text);
	...
	private void addBaseImage(View view, ArrayList<BaseProduct> bannerArray){
		baseInfoImageLayout = (LinearLayout) view.findViewById(R.id.base_info_image_linear_layout);
...
//			ImageView matchchild = (ImageView) getSherlockActivity().getLayoutInflater().inflate(R.layout.item_base_image, null);
//			ImageView child = (ImageView) getSherlockActivity().getLayoutInflater().inflate(R.layout.item_base_image, null);
//			ImageView child = new ImageView();
//			ImageView matchchild = new ImageView(getSherlockActivity());
//			int base_px = getResources().getDimensionPixelSize(R.dimen.base_info_image);
			
//			int imageDimen = (int) getResources().getDimension(R.dimen.base_info_image);
//			lp_base.setMargins(imageDimen, imageDimen, imageDimen, imageDimen);
//			LinearLayout.LayoutParams lp_match = new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT,
//					LayoutParams.MATCH_PARENT, 1);
//			int matchDimen = (int) getResources().getDimension(R.dimen.base_info_match);
//			lp_match.setMargins(matchDimen, matchDimen, matchDimen, matchDimen);
...
//			imageViewLayout.setPadding(10, 0, 10, 0);
	...
	private void addMatchImage(View view, final ArrayList<MainProduct> bannerArray){
		matchGoodsImageLayout = (LinearLayout) view.findViewById(R.id.match_goods_image);
		int lenght = bannerArray.size();
		Resources r = activity.getResources();
		float px = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 90, r.getDisplayMetrics());
		for(int i = 0; i < lenght; i++){
			LinearLayout.LayoutParams lp_base = new LinearLayout.LayoutParams((new Float(px)).intValue(),
					LayoutParams.WRAP_CONTENT);
//			View imageViewLayout = LayoutInflater.from(view.getContext()).inflate(R.layout.goods_banner_imageview, null);
//			ImageView bannerImageView = (ImageView) imageViewLayout.findViewById(R.id.banner_imageview);
			ImageView bannerImageView = new ImageView(activity);
			bannerImageView.setLayoutParams(lp_base);
...
//			TextView nameTextView = (TextView) imageViewLayout.findViewById(R.id.banner_name);
//			nameTextView.setText(bannerArray.get(i).getTitle());
//			TextView priceTextView = (TextView) imageViewLayout.findViewById(R.id.banner_price);
